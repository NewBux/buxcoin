...
const string strMessageMagic = "December Buxcoin Signed Message:\n";
...
{
    int64_t nSubsidy = nSubsidy333;
    int64_t nSubsidy1 = 450;
    int64_t nSubsidy5 = 14;
    int64_t nSubsidy3 = 1000000*COIN;
    int64_t nSubsidy2 = 10;

  if(nBestHeight > 0  && nBestHeight  <=100 )
  {
        if(nBestHeight == 25)
        {
            return  ((nSubsidy1 *  nSubsidy3)  + nFees );
         }
       return  nSubsidy2 * COIN   + nFees ;
 }
   if(nBestHeight > 100){
 	 //  nSubsidy >>= (nBestHeight / 10000);
    ...
    return   nFees +nFees+nFees +nFees;
}
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Bux", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "The times 01.12.2019. December Buxcoin project is here.";
        CTransaction txNew;
        txNew.nTime = 1575222491;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1575222491;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1575978348 : 1575978348 ;


if (1==2  && (block.GetHash() != hashGenesisBlock)) {
...
printf("block.nNonce = %u \n", block.nNonce);
        ...
        assert(block.hashMerkleRoot == uint256("0xa8d9e87553ddd1e01e7234991d1dbcc86219f17947a8120ec1b28c8b869c768f"));
